...
class Flight : public rclcpp::Node {
  public:
    Flight() : Node("flight") {
      ...
      lla_sub_ = this->create_subscription<VehicleGlobalPosition>("/fmu/out/vehicle_global_position", rclcpp::SensorDataQoS(),
  ...
  private:
    ...
    rclcpp::Subscription<VehicleGlobalPosition>::SharedPtr lla_sub_;
    rclcpp::Subscription<VehicleLandDetected>::SharedPtr landed_sub_;

    px4_msgs::msg::VehicleOdometry curr_odom_;
    px4_msgs::msg::VehicleGlobalPosition curr_global_;
    ...
    std::vector<Eigen::Vector3d> lla_waypoints_ = {
      {47.397971490869324, 8.546163669065939, 2.237056210637093},
      {47.39798945717559, 8.546163669065939, 2.237056210637093},
      {47.397971490869324, 8.546163669065939, 2.237056210637093},
      {47.397971490869324, 8.54619020129361, 2.237056210637093},
      {47.397971490869324, 8.546163669065939, 2.237056210637093},
      {47.397971490869324, 8.546163669065939, 3.237056210637093},
    };
    ...
    Eigen::Vector3d lla_origin = Eigen::Vector3d::Zero();
    GeographicLib::LocalCartesian local_cartesian_;
    bool set_origin_done = false;
    int origin_counter = 0;
    int origin_count_threshold = 10;
    
    void convert_lla_to_ned();
};

void Flight::convert_lla_to_ned(){
  waypoints_.clear();
  
  for(const auto &lla : lla_waypoints_) {
    double east, north, up;
    
    local_cartesian_.Forward(lla[0], lla[1], lla[2], east, north, up);
    
    waypoints_.push_back({static_cast<float>(north), static_cast<float>(east), static_cast<float>(-up)});
  }
}

void Flight::set_origin(){
  if(set_origin_done = true) return;
  
  origin += Eigen::Vector3f(curr_odom_.position[0], curr_odom_.position[1], curr_odom_.position[2]);
  lla_origin += Eigen::Vector3d(curr_global_.lat, curr_global_.lon, curr_global_.alt);
  origin_counter ++;
  
  if(origin_counter == origin_count_threshold) {    
    origin /= origin_count_threshold;
    lla_origin /= origin_count_threshold;

    set_origin_done = true;
    RCLCPP_INFO(this->get_logger(), "Origin set to (%f, %f, %f)", origin[0], origin[1], origin[2]);
    RCLCPP_INFO(this->get_logger(), "LLA origin set to (%lf, %lf, %lf)", lla_origin[0], lla_origin[1], lla_origin[2]);
