...
class Target : public rclcpp::Node {
  ...
  private:
    ...
    float desired_yaw_ = 0.0f; //North 0, CCW(+), [rad] //for rescue related
    ...
    float setpoint_timeout_s_ = 0.3f;
    float descent_slew_mps_ = 0.05f;
...
void Target::land() {
  ...
  float descent_mps = select_descent_speed(alt_m, valid_xy);

  if (descent_mps > descent_cmd_) {
    descent_cmd_ = std::min(descent_mps, descent_cmd_ + descent_slew_mps_);
  } else {
    descent_cmd_ = std::max(descent_mps, descent_cmd_ - descent_slew_mps_);
  }
  descent_mps = descent_cmd_;
