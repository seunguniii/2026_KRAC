[README.md](https://github.com/user-attachments/files/29348075/README.md)
...
| 파일 | 노드 이름 | 역할 |
|---|---|---|
| `marker_recognition_x500.py` | `marker_recognition` | 카메라 영상 수신, ArUco 검출, 거리센서 고도 측정, 마커 상대좌표 계산, `/landing/coordinates` publish |
| `land_test.cpp` | `landing` | `/landing/coordinates`를 받아 PX4 Offboard setpoint 생성, 위치/속도 기반 접근, 하강 제어, 최종 NAV_LAND 명령 |

---

## 2. `marker_recognition_x500.py` 설명
...
카메라는 RTSP stream을 GStreamer pipeline으로 받아온다.
...
`land_test.cpp`는 PX4 Offboard 제어 노드이다. 이 노드는 `/landing/coordinates`에서 받은 body 기준 상대 위치 오차를 이용해 드론을 ArUco marker 중심으로 접근시키고, 조건이 만족되면 하강 및 최종 착륙 명령을 보낸다.
